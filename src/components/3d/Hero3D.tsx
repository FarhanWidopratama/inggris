...
import { Float, PerspectiveCamera, ContactShadows, Environment, OrbitControls } from "@react-three/drei";
...
function Scene({ isMobile }: { isMobile: boolean }) {
  return (
    <>
      ...
      {/* 3 floating books - moved UP so tidak ketutup kartu */}
      <FloatingBook position={isMobile ? [-1.05, 0.65, 0.35] : [-1.4, 0.78, 0.45]} rotation={[0.18, -0.42, 0.09]} color="#059669" scale={isMobile ? 0.78 : 0.98} />
      <FloatingBook position={isMobile ? [0.08, 1.05, -0.35] : [0.12, 1.28, -0.35]} rotation={[0.12, 0.18, -0.06]} color="#0284c7" scale={isMobile ? 0.68 : 0.84} />
      <FloatingBook position={isMobile ? [1.05, 0.42, 0.45] : [1.42, 0.62, 0.55]} rotation={[-0.12, 0.48, 0.07]} color="#d97706" scale={isMobile ? 0.70 : 0.88} />

      {/* Floating letter blocks - naik juga */}
      <FloatingLetter position={isMobile ? [-0.65, -0.42, 1.05] : [-0.85, -0.38, 1.15]} color="#0f172a" />
      <FloatingLetter position={isMobile ? [0.18, -0.55, 0.95] : [0.22, -0.48, 1.05]} color="#10b981" />
      <FloatingLetter position={isMobile ? [0.88, -0.28, 0.98] : [1.08, -0.22, 1.12]} color="#0ea5e9" />
      ...
      <OrbitControls enableZoom={false} enablePan={false} autoRotate autoRotateSpeed={0.45} minPolarAngle={Math.PI / 3.2} maxPolarAngle={Math.PI / 1.95} minAzimuthAngle={-0.6} maxAzimuthAngle={0.6} />
    </>
  );
}
...
export default function Hero3D() {
  ...
  return (
    <div className="relative h-[340px] w-full rounded-[24px] sm:h-[460px] sm:rounded-[28px]" style={{ perspective: 1000 }}>
      {/* bg - rounded but not clipping canvas */}
      <div className="absolute inset-0 overflow-hidden rounded-[24px] bg-gradient-to-br from-zinc-900 via-[#0f172a] to-black sm:rounded-[28px]">
      ...
      {/* Canvas - allow overflow so books not clipped by rounded */}
      <Canvas
        shadows={!isMobile}
        dpr={dpr}
        gl={{ antialias: !isMobile, alpha: true, powerPreference: isMobile ? "low-power" : "high-performance" }}
        className="!absolute inset-0 rounded-[24px] sm:rounded-[28px]"
