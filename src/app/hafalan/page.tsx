...
export default function HafalanPage() {
  const allVocab = useMemo(() => lessons.flatMap((ls) => ls.vocab.map((v) => ({ ...v, lessonId: ls.id, lessonTitle: ls.title }))), []);
  ...
  const filtered = allVocab.filter((v) => {
    ...
    const matchF = filter === "all" || v.lessonId.startsWith(filter.split("-")[0] + "-");
  ...
  return (
    <div className="mx-auto max-w-6xl px-4 py-6 sm:px-6">
      <h1 className="text-2xl font-black tracking-tight">Hafalan Wajib — 500 Kata EI em</h1>
      <p className="text-sm text-zinc-600">Semua vocab dari 60 lesson. Search, dengerin native, hafal 10/hari. Waveform ada di Speaking Lab.</p>

      <div className="mt-4 flex flex-wrap gap-2">
        <input value={q} onChange={(e) => setQ(e.target.value)} placeholder="Cari: Hello, Thank you, EI em..." className="flex-1 min-w-[200px] rounded-full border border-zinc-200 bg-white px-4 py-2.5 text-sm" />
        <select value={filter} onChange={(e) => setFilter(e.target.value)} className="rounded-full border border-zinc-200 bg-white px-4 py-2.5 text-sm font-bold">
          <option value="all">Semua Level (60 lesson)</option>
